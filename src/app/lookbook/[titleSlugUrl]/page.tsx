    ...
    const fetchCollection = async () => {
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_STRAPI_API_URL}/api/collections?filters[titleSlugUrl][$eq]=${titleSlugUrl}&populate=*`
        );
    ...
    <>
      ...
      <div className="h-screen md:h-[100vw] lg:h-screen flex flex-col items-center justify-center px-[5vw] md:px-[10vw] lg:px-[15vw] pt-20 md:pt-28">
        ...
        <div className="w-full md:w-2/3 lg:w-2/5 flex flex-col md:flex-row items-center justify-center gap-4 md:gap-8 lg:gap-12">
          ...
          <img
            src={`${process.env.NEXT_PUBLIC_STRAPI_API_URL}${collection.Images[prevIndex].url}`}
            alt="Previous Image"
          ...
          {/* Main Image */}
          <img
            src={`${process.env.NEXT_PUBLIC_STRAPI_API_URL}${collection.Images[currentIndex].url}`}
          ...
          <img
            src={`${process.env.NEXT_PUBLIC_STRAPI_API_URL}${collection.Images[nextIndex].url}`}
            alt="Next Image"
            className="hidden md:block w-[15vw] object-cover opacity-50 cursor-pointer hover:opacity-75 transition-opacity"
